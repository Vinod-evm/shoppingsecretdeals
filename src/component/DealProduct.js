...
const DealProduct = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const header = document.getElementById("joinTele");
      if (!header) return; // Element not found, exit early
  
      const sticky = header.offsetTop;
  
      if (initialOffset === null) {
        initialOffset = sticky; // Store the initial offset when sticky behavior starts
      }
  
      if (window.pageYOffset >= initialOffset) { // Use 'greater than or equal to' to ensure correct behavior
        header.classList.add("sticky");
      } else {
        header.classList.remove("sticky");
        initialOffset = null; // Reset the initial offset when scrolling back up
      }
    };
