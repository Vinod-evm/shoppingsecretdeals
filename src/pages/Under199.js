  ...
  const fetchPosts = () => {
    ...
    fetch(`https://shoppingsecretdeals.com/wp-json/wp/v2/posts?per_page=${perPage}&page=${currentPage}`)
      ...
      .then((data) => {
        ...
        // Filter the data to include only products with a price less than or equal to ₹199
        const filteredData = data.filter((product) => {
          const priceString = product.meta.rehub_offer_product_price;
          console.log(priceString,'priceString');
          const price = extractPrice(priceString);
          return price <= 199; // Ensure correct comparison for prices equal to or less than ₹199
        });
